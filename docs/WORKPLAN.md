...
# Portability gate — core compiles for every deployment target (browser,
# Unity WebGL/iOS/Android). A failure means a host/OS dependency leaked in.
...
- **Status:** DONE (branch `combat`, off `movement`, 2026-07-03; four green gates + iOS/Android/WebGL target checks, 297 tests; all five guardians cleared, rules-guardian PASS; opened CMB-CONST). Not yet committed.
...
- **Status:** DROPPED (2026-07-04, per pinned architecture decision). Hosts live
  in their own repos, not here: the **server is a separate SpacetimeDB repo**
  consuming `mu-core` as a library, and any Unity/browser client is likewise its
  own repo. This crate stays **pure core** — no in-repo host/harness. Portability
  is already proven without one: the CI compile-gate covers
  `wasm32-unknown-unknown` (SpacetimeDB's own module target) plus
  `wasm32-unknown-emscripten`/`aarch64-apple-ios`/`aarch64-linux-android` (Unity),
  RNG/tick are injected, deps are frozen at `serde`+`rand_core`, and the
  `core/tests/*_simulation.rs` suites double as usage examples. Next work is core
  game systems (below), never hosts. The section below is retained for historical
  context only.
...
- **Status:** DONE (2026-07-04; closed Q1/Q2/Q3/Q4 + T1; four green gates + all
  four portability target checks + `cargo xtask scan` clean + wasm determinism
  run under wasmtime). Not yet committed.
...
| 2026-07-03 | W-CMB | Started on branch `combat` (off `movement`). Core Domain Feature pipeline: spec → architecture(plan) → canon(plan) → state-machine → implementation → architecture(code) → deep-module → canon(code) → debt → rules. Confirmed the **combatant combat-profile gap** (W-ENT shipped raw `Stats`/`Level`/`Exp`/`Vitals` only; no derived combat stats, no `CombatBonus` aggregation, no equipment on `Character`) — flagged as the #1 plan-phase pressure-test. Confirmed `Atlas::parse` discards `game_config.progression` (needs an `Atlas::progression()` accessor this wave). |
| 2026-07-03 | W-CMB | **On-hit effect completeness sweep (user-requested).** Found a missed effect family: `SkillShape::Lunge` = the 5 DK weapon skills (data #19–23) carry `MovesToTarget` (attacker dashes to target) + `MovesTarget` (target knocked back on hit) per facts doc 3 L32–33 — the plan had resolved Lunge as a plain DirectHit. Folded BOTH displacements into scope, composing the movement service (same mechanic as the lightning shove). All other on-hit effects confirmed covered (elemental ailment-set gated by resistance; crit/excellent/ignore-defense/double; lightning shove) or correctly out (leech/reflect/stun/combo = S3+/equipment). Per-skill multi-hit/projectile counts absent from the data model (deliberate) → one hit per target. |
| 2026-07-03 | W-CMB | **Plan phase locked (both plan-guardians APPROVE-WITH-REQUIRED-CHANGES).** Design: symmetric `CombatProfile` (data in `components/combat_profile.rs`) + per-class derivation (rule in `services/profile.rs`); `resolve_attack`/`cast`/`resolve_kill_drops`/`award_kill_experience`/`resolve_kill` pure services; `AttackOutcome` axis-split (lethality `Missed\|Landed\|Killed` × `HitQuality` × `DamageModifiers` ClassSet-style set, no flag-soup); `DamagingSkill` subset + 18-arm `AreaPattern`→`Region`; loot `DropCategory` cumulative walk (window `[ml-11, ml]`, money = exp+7); exp term-by-term + level-up loop. Two load-bearing canon corrections: (1) derivation must be **pooled single-divide** per stat (not per-term truncation) — corrects two derived values (MG/DL max-ability 34→35); (2) elemental application needs a **faithful `roll_apply_elemental` = (r<255) && uniform_below(r+1)==0** (the existing `roll_resistance` byte/255 curve is ~24× off mid-range) — gates ailment AND lightning shove. New: `Pool::reduced`, `Interval::spanning`, `chance::{uniform_in_inclusive, roll_apply_elemental}`, shared `scale_ratio`, `Atlas::progression()`. Hardcoded consts (3% floor, 3/10 overrate, level/10 floor, 5/4 exp, money+7, window gap 11) → W-SRC provenance row (debt-guardian). |
| 2026-07-03 | (portability) | **Unity/mobile target gates added (user-requested).** Verified the pure core compiles for every deployment target, not just browser wasm: `wasm32-unknown-unknown` (browser), `wasm32-unknown-emscripten` (Unity WebGL), `aarch64-apple-ios` (Unity iOS), `aarch64-linux-android` (Unity Android). Wired all four `cargo check` legs into `.github/workflows/ci.yml`, README Development, and the WORKPLAN four-gates block. The Rust↔C# FFI shim remains a future host wave. |
| 2026-07-03 | W-CMB | **Landed.** `components/combat_profile.rs` (`CombatProfile`/`VitalMaxima`/`CombatTarget`); services `ratio`/`profile`/`combat`/`skills`/`loot`/`experience`/`kill`; events `combat`/`skills`/`loot`/`progression`/`kill`; `chance::{uniform_in_inclusive, roll_apply_elemental}`; `Pool::reduced`/`Interval::spanning`/`ItemLevel::clamped`; `Atlas::progression()`. Pooled per-class derivation (MG/DL max-ability=35); faithful `roll_apply_elemental`; `resolve_attack` grid-free with fixed RNG order + three-axis `AttackOutcome`; lightning shove **and** the DK-lunge dash+knockback composed only in `skills::cast`; drop partition + window + specials; exp term-by-term + level-up climb; real-Atlas e2e kill-loop + mutation teeth-check. Four green gates + all four target checks, **297 tests** (+89 net). All five guardians cleared: arch/deep-module/canon PASS; two must-fixes applied at root (fabricated-zero `bounding_rect` → array destructure; dead `resistances()` accessor deleted); **rules-guardian final PASS** on independent re-audit. |
| 2026-07-03 | W-CMB | Opened **CMB-CONST** (`docs/debt/combat-constants-provenance.md`, owner W-SRC) for 13 hardcoded/invented combat/exp/drop/displacement constants (3% hit floor, 3/10 overrate, level/10 floor, 5/4 exp, over-level gap 10, high-level victim 65, money+7, window gap 11, `DASH_SPEED`=8t, `SINGLE_TARGET_RADIUS`=1t, `ONE_TILE_SPEED`), each carrying a `// W-SRC:` provenance comment — same species as MOB-SPD but in-code. DEBT-INDEX row added. Resolves no existing debt. Wave not yet committed (awaiting user). |
| 2026-07-03 | W-CMB | **Scope decisions pinned (user).** (1) **Player fight-math IS in scope** — W-CMB defines the `CombatProfile` port and builds the base per-class stat→combat derivation for all 8 `CharacterClass` variants (5 formula families) from the *sourced* facts (`docs/reference/openmu-facts/0…md`: attack-rate `5·Lv + 1.5·Agi + 0.25·Str`, per-class phys min/max from Str, `DefenseFinal = 0.5·DefenseBase`, wizardry from Ene, HP/Mana/AG formulas; classic-PvP so PvM rates are authoritative), matching the `openmu-reference.md` "bespoke per-class integer-coefficient function" mandate. Gearless character → crit/excellent/defense-ignore/double chances are **authentic zero** (from excellent options only), not fabricated defaults. Equipment/option/set `CombatBonus` aggregation + level-up vitals recompute stay deferred (need Inventory/Equipment). (2) **Lightning 1-tile displacement IS in scope** — modeled as an authentic reposition composing `movement::resolve_step`; "target can't attack while shoved" is **emergent** (shoved out of range → AI must re-close before attacking), no invented stun timer (none is sourced pre-S3). Fractional coefficients evaluated as exact integer ratios (widen→mul→integer-div→narrow), never float. |
| 2026-07-04 | W-HARDEN | Started (tooling/CI wave, no domain prerequisite). Refactor/Cleanup pipeline: architecture → deep-module → canon → debt → rules. Confirmed via survey that the four Q3 bans are absent from the clean tree (the four `unwrap_or` are on `try_from`, not lookup-shaped, so correctly not flagged). |
| 2026-07-04 | W-HARDEN | **Landed.** **Q1** `core/tests/wire_drift.rs` — one canonical exact-JSON pin per host-facing wire type (spatial types + `Region` circle/rect/cone; vocabulary newtypes/enums; `SpecialDrop`; the W-ENT/W-MOV/W-CMB event enums). **Q2** `proptest!` blocks in `services/chance.rs` (weight-sum, real-bucket, monotonicity, `pick_one` membership+coverage) and a new test module in `rng/mod.rs` (`uniform_below`/`_usize < bound`, equidistribution band), plus `components/units.rs` (`Level`/`Zen`/`Exp` range round-trips + out-of-range reject). **Q3** new `xtask/` member (`syn` visitor) flags the four review-enforced bans in `core/src` with `file:line`/non-zero exit; `cargo xtask` alias; `.githooks/pre-commit`; README documents `git config core.hooksPath .githooks`; `xtask` omits workspace lints and never enters core's dep graph. **Q4** `.github/workflows/ci.yml` → ubuntu/macOS/windows matrix (fmt/clippy/test + `cargo xtask scan`) + kept 4 portability compile-checks + a wasmtime leg RUNNING `cargo test --test wasm_determinism --target wasm32-wasip1`. **T1** `narrow_u8` dead arm removed at root via total `tile_index` (clamp + cast-free low-byte read). Four green gates, `xtask scan` clean, all four target checks, wasm determinism run under wasmtime 46 (native==wasm bit-identical). |
| 2026-07-04 | W-HARDEN | Closed **Q1/Q2/Q3/Q4** (`practices-transfer-quality.md` record fully closed) and **T1** (`spatial-foundation-followups.md` fully closed — last open sub-item). Five rows removed from `DEBT-INDEX.md`. |
| 2026-07-04 | W-HARDEN | **Q4 fallback pinned.** The full `cargo test --target wasm32-wasip1` is not wasi-buildable — `proptest` transitively pulls `wait-timeout`, which fails to compile for wasi. Per the workplan's recommended fallback: `proptest` gated to `cfg(not(target_arch = "wasm32"))` dev-deps (property tests run on native legs), and a dedicated proptest-free determinism test (`core/tests/wasm_determinism.rs`, fixed seed → fixed hardcoded output) is the leg wasmtime executes. |
| 2026-07-04 | W-INV | Started on branch `inventory` (off `main`), **in parallel with W-EFFECT** (separate session; strict no-shared-file rule — only `*/mod.rs` `pub mod` additions shared, git auto-merges). Core Domain Feature pipeline: spec → architecture(plan) → canon(plan) → state-machine → implementation → architecture(code) → deep-module → canon(code) → debt → rules. |
| 2026-07-04 | W-INV | **Plan phase locked (both plan-guardians APPROVE-WITH-REQUIRED-CHANGES).** Key reshapes: (1) **`ItemRef` relocated** `data/common.rs` → new `components/item_ref.rs`, re-exported (`pub use`) so `data::common::ItemRef` is byte-frozen — the MapNumber precedent, forced because a component (`ItemInstance`) may not import `data`; (2) rolled options modeled as a **sum-over-rarity** `RarityRoll { Normal \| Excellent{options} \| Ancient{bonus} }` making excellent-only-on-excellent + ancient-only-on-ancient unrepresentable; (3) excellent options = a **non-empty, duplicate-free `NonZeroU8` bitmask** keyed by the existing `slot_index()` (drops the derivable `WeaponDamageKind`); (4) **`ItemInstance` is move-only** (not `Copy`) — dupe-safety; every reject arm hands the item back; (5) new component **`EquipSlot`** enum (can't import `data::EquipmentSlot`), one 12-arm translation in the equip service; (6) dedicated u8 `Durability`; two-variant `LuckRoll`/`SkillRoll`; component-owned `PlacementRejection`; the roll service takes rarity+level as INPUTS from `Drop::Item` (loot.rs frozen — never re-rolled). |
| 2026-07-04 | W-INV | **Landed.** New components `item_ref`/`item_instance` (`ItemInstance`,`RarityRoll`,`ExcellentOptions`,`ExcellentArmorSet`/`WeaponSet`,`RolledNormalOption`,`LuckRoll`,`SkillRoll`,`Durability`,`reconcile`)/`inventory` (`Cell`,`Footprint`,`PlacedItem`,`Inventory`,`PlacementRejection`, AABB grid ops)/`equipment` (`EquipSlot`, slot-keyed `Equipment`); entity `world_item` (`WorldItem`); services `item_roll` (`roll_dropped_item` + per-kind total gates + fixed RNG draw-order + Fisher-Yates distinct excellent picks) & `inventory` (place/remove/move/pickup/equip/unequip); events `inventory`. Containers stand alone (NOT yet on `Character` — deferred 5-min merge). Five green gates (fmt, clippy `-D warnings`, `test --workspace`, all four portability targets, `xtask scan`), **+8 real-Atlas integration tests** (roll across seeds, grid/equip invariants, determinism bit-equality, zero-word jewels, loot→roll bridge) + wire-drift & wasm-determinism pins. All five guardians cleared; **rules-guardian final PASS** on independent gate re-run. Not committed (awaiting user). |
| 2026-07-04 | W-INV | Opened **I1** (two-handed weapon doesn't block paired hand — root needs a reconcile-with-defs reload pass, owner W-EFFECT), **I2** (`EquipSlot` twin of `data::EquipmentSlot` — collapses via the ItemRef relocation on the next `game_config.rs` touch, owner W-ENT), **I3** (`loot::item_drop` can pair Excellent with a non-excellent-capable kind → roll degrades to Normal; root is a loot-side pool filter, owner W-EFFECT) in `docs/debt/inventory-item-wave-followups.md` + DEBT-INDEX. Prior-wave `max_durability` rarity bonus routed to CMB-CONST (not W-INV debt). Character-wiring + CombatBonus→profile deferrals ruled clean (roadmap-owned, no row). Resolves no existing debt. |
| 2026-07-04 | W-INV | **Closed I1/I2/I3** (root fixes landed on `main`, user-directed — the parallel-wave file freeze lifted). **I2:** slot enum de-duplicated — `EquipmentSlot` relocated `data/game_config.rs` → `components/equipment.rs`, re-exported via `pub use` (public path byte-frozen; `EquipSlot` twin + 12-arm `translate_slot` deleted). **I3:** `loot::item_drop` now gates the excellent pool on `is_excellent_capable(kind)` (shared from `item_roll`), so an excellent drop never lands on a non-excellent-capable kind and the roll's `None`-arm is unreachable for authentic drops. **I1:** two-handed dual-hand occupancy enforced — `equip` (now taking `&Atlas` to resolve the paired hand) rejects 2H↔offhand with `EquipRejection::TwoHandedConflict`, plus a reload-boundary `reconcile_equipment` → `EquipmentConflict::TwoHandedWithOffhand`; `hand_occupation` reads `Weapon{TwoHanded}`\|`Bow`\|`Crossbow` as two-handed (bow/crossbow structural, `// W-SRC:`-noted; Staff-2H flagged as a future data-model refinement). Five green gates (fmt, clippy `-D warnings`, **414 tests**, wasm target, xtask scan); all five guardians cleared, **rules-guardian final PASS**. Rows removed from DEBT-INDEX; `inventory-item-wave-followups.md` closed. Opened **EXC-DROP-WINDOW** (`excellent-drop-level-window.md`, owner W-SRC) — a PRE-EXISTING W-CMB gap canon surfaced: `item_drop` draws the excellent pool from the Normal `[level-11, level]` band, missing OpenMU's `monster_level>=25` gate + `level-25` pool band (not an I1–I3 regression). |
